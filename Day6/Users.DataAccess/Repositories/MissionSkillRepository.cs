...
namespace Users.DataAccess.Repositories
{
    public class MissionSkillRepository
    {
        ...
        {
            return _missionDbContext.MissionSkiils.ToList();
        }

        public MissionSkiil GetMissionSkillById(int id)
        {
            MissionSkiil user = _missionDbContext.MissionSkiils.FirstOrDefault(u => u.Id == id);
            if (user == null)
            {
                return null;
            }
        ...
        {
            List<MissionSkiil> users = _missionDbContext.MissionSkiils.Where(u => u.SkillName.Equals(skill)).ToList();
            return users;
        }

        public void AddMissionSkill(MissionSkiil mSkill)
        {
            _missionDbContext.MissionSkiils.Add(mSkill);
        ...
        public int DeleteMissionSkill(int id)
        {
            ...
            {
                _missionDbContext.MissionSkiils.Remove(mSkillDelete);
                _missionDbContext.SaveChanges();
                return 1;
            }
        }
