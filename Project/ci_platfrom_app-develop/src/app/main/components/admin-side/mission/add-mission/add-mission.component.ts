...
export class AddMissionComponent implements OnInit, OnDestroy {
      ...
      .subscribe(
        (data: any) => {
          if (data.result == 1) {
            ...
            this.missionSkillList = data.data.map((item: any) => ({
              value: item.id,
              text: item.skillName,
            }));
          } else {
      ...
      .subscribe(
        (data: any) => {
          if (data.result == 1) {
            ...
            this.missionThemeList = data.data.map((item: any) => ({
              value: item.id,
              text: item.themeName,
            }));
          } else {
  ...
  async onSubmit() {
    ...
    if (this.addMissionForm.valid) {
      ...
      //   this._toast.error({
      //     detail: 'ERROR',
      //     summary: 'Please upload at least one image.',
      //     duration: APP_CONFIG.toastDuration,
      //   });
      //   return;
      // }
      console.log('Image URLs from server:', imageUrl); // Should be array of image paths
      let imgUrlList = imageUrl.map((e) => e.replace(/\s/g, '')).join(',');
      value.missionImages = imgUrlList;
      console.log('Form data to send:', value); // Make sure missionImages is present
