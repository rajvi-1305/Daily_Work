...
export class VolunteeringMissionComponent implements OnInit, OnDestroy {
  ...
  fetchMissionDetail(missionId: any) {
    ...
    const missionDetailSubscribe = this._service
      .missionDetailByMissionId(value)
      .subscribe(
        (data: any) => {
          if (data.result == 1) {
            ...
            if (this.missionDetail.missionDocuments) {
              this.missionDoc =
                this._service.imageUrl +
                '/' +
                this.missionDetail.missionDocuments;
            }
            this.btnText =
  ...
  postComment(commentdesc: any) {
    ...
    if (tokenDetail == null || tokenDetail.userType != 'user') {
      ...
      const value = {
        ...
        commentDate: format(new Date(), 'yyyy-MM-DDTHH:mm:ssZ'),
      };
      const missionCommentSubscribe = this._service
        .addMissionComment(value)
        .subscribe(
          (data: any) => {
            if (data.result == 1) {
              this._toast.success({
  ...
  getMissionCommentList() {
    const missionCommentSubscribe = this._service
      .missionCommentListByMissionId(this.missionDetail.id)
      .subscribe(
        (data: any) => {
          if (data.result == 1) {
  ...
  getMissionFavourite(missionId: any) {
    ...
    if (tokenDetail == null || tokenDetail.userType != 'user') {
      ...
      if (this.missionFavourite) {
        const addMissionFavouriteSubscribe = this._service
          .addMissionFavourite(value)
          .subscribe((data: any) => {
            if (data.result == 1) {
        ...
        const removeMissionFavouriteSubscribe = this._service
          .removeMissionFavourite(value)
          .subscribe((data: any) => {
            if (data.result == 1) {
              this.fetchMissionDetail(missionId);
            } else {
  ...
  getRecentVolunteerList() {
    ...
    const volunteerListSubscribe = this._service
      .recentVolunteerList(value)
      .subscribe((data: any) => {
        if (data.result == 1) {
